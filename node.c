...
static LV2_Worker_Status xxx_worker_respond(LV2_Worker_Respond_Handle handle,
                                            const uint32_t size,
                                            const void *data) {
  ...
  if (size > MAX_WORK_RESPONSE_MESSAGE_SIZE) {
    ...
    spa_ringbuffer_get_write_index(&node->host.work_response_ring,
                                   &write_index);
    ...
    spa_ringbuffer_write_update(&node->host.work_response_ring,
                                write_index + total_len);
  }

  return LV2_WORKER_SUCCESS;
}

static int on_worker(struct spa_loop *loop, bool async, uint32_t seq,
                     const void *data, size_t size, void *user_data) {
  struct node_data *node = (struct node_data *)user_data;
  // printf("\nloop_worker");fflush(stdout);
  LV2_Worker_Status x = node->host.iface->work(
      node->host.handle, xxx_worker_respond, node, size, data);
  // printf("\nworker status %d",x);fflush(stdout);
  return x;
}

static LV2_Worker_Status my_schedule_work(LV2_Worker_Schedule_Handle handle,
                                          uint32_t size, const void *data) {
  struct node_data *node = (struct node_data *)handle;
  // printf("\nHost: schedule_work()");fflush(stdout);
  //  Fire execution of on_worker in loop thread
  pw_loop_invoke(pw_thread_loop_get_loop(node->pw.loop), on_worker, 0, data,
                 size, false, node);
  return LV2_WORKER_SUCCESS;
}
...
static void on_process(void *userdata, struct spa_io_position *position) {
  ...
  if (!node->pw.connected) {
    ...
    if (node->host.start_ui)
      pw_loop_invoke(pw_thread_loop_get_loop(node->pw.loop), pluginui_on_start,
                     0, NULL, 0, false, node);
  }
  ...
  for (int n = 0; n < node->n_ports; n++) {
    ...
    if (port->pre_run)
      port->pre_run(port, node, frame, denom, (uint64_t)n_samples);
  }
  ...
  /////
  {
    ...
    while ((write_index - read_index) >= sizeof(uint16_t)) {
      ...
      offset =
          (read_index + sizeof(uint16_t)) & (WORK_RESPONSE_RINGBUFFER_SIZE - 1);
...
static int on_preset(struct spa_loop *loop, bool async, uint32_t seq,
                     const void *data, size_t size, void *user_data) {
  struct node_data *node = (struct node_data *)user_data;
  ...
  if (strlen(preset_uri)) {
    ...
    if (node->host.lilv_preset) {
      ...
      LilvState *state = lilv_state_new_from_world(
          constants.world, &constants.map, node->host.lilv_preset);
      if (state) {
...
static void on_command(void *data, const struct spa_command *command) {
  ...
  if (id == SPA_NODE_COMMAND_User) {
    ...
    if (sscanf(command_string, "preset %s", uri) == 1) {
      pw_loop_invoke(pw_thread_loop_get_loop(node->pw.loop), on_preset, 0, uri,
                     strlen(uri)+1, false, node);
    } else {
...
void on_param_changed(void *data, void *port_data, uint32_t id,
...
static int on_start(struct spa_loop *loop, bool async, uint32_t seq,
                    const void *data, size_t size, void *user_data) {
  struct node_data *node = (struct node_data *)user_data;
  ...
  if (!strlen(node->nodename))
    strcpy(node->nodename, strdup(lilv_node_as_string(
                               lilv_plugin_get_name(node->host.lilvPlugin))));

  pw_loop_add_signal(pw_thread_loop_get_loop(node->pw.loop), SIGINT, do_quit,
                     node);
  pw_loop_add_signal(pw_thread_loop_get_loop(node->pw.loop), SIGTERM, do_quit,
                     node);

  node->pw.filter = pw_filter_new_simple(
      pw_thread_loop_get_loop(node->pw.loop), node->nodename,
  ...
  if (pw_filter_connect(node->pw.filter,
                        PW_FILTER_FLAG_RT_PROCESS | PW_FILTER_FLAG_DRIVER,
                        params, 1) < 0) {
...

  {
    ...
    if (lilv_plugin_has_feature(node->host.lilvPlugin,
                                constants.worker_schedule)) {
    ...
    node->host.options[0] = (LV2_Options_Option){
        ...
        constants.atom_Int,
        &min_block_length};
    node->host.options[1] = (LV2_Options_Option){
        LV2_OPTIONS_INSTANCE,
        0,
        ...
        constants.atom_Int,
        &max_block_length};
    node->host.options[2] = (LV2_Options_Option){
        LV2_OPTIONS_INSTANCE,
        0,
    ...
    node->host.options[4] = (LV2_Options_Option){
        LV2_OPTIONS_INSTANCE,
        0,
        ...
        &fsample_rate};
    node->host.options[5] =
        (LV2_Options_Option){LV2_OPTIONS_INSTANCE, 0, 0, 0, 0, NULL};
    ...
    node->host.instance = lilv_plugin_instantiate(
        node->host.lilvPlugin, node->pw.samplerate, node->host.features);

    node->host.handle = lilv_instance_get_handle(node->host.instance);

    node->host.worker_data = NULL;
    if (node->host.instance != NULL) {
      if (lilv_plugin_has_extension_data(node->host.lilvPlugin,
                                         constants.worker_iface)) {
        node->host.iface =
  ...
  for (int n = 0; n < node->n_ports; n++) {
    struct port_data *port = &node->ports[n];
    if (port->setup)
      port->setup(port, node);
  }

  lilv_instance_activate(node->host.instance);
   pthread_mutex_unlock(&program_lock);

  pw_loop_invoke(pw_thread_loop_get_loop(node->pw.loop), on_preset, 0, node->preset_uri, sizeof(node->preset_uri),
                 false, node);
...
void node_set_latency(struct node_data *node, int period) {
  node->pw.latency_period = period;
}
...
printf("\nnode start [%s] %lx   %lx",node->preset_uri,node->preset_uri,node);
  ...
  pw_loop_invoke(pw_thread_loop_get_loop(node->pw.loop), on_start, 0, NULL, 0,
                 false, node);
}
...
void node_set_ui_show(struct node_data *node, int show_ui) {
  node->host.start_ui = show_ui;
}
