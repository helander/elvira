...
void util_print_atom(const LV2_Atom *atom) {
         if (atom->type != 0) {
            uint8_t *p = (uint8_t *) (atom + 1);
            printf("\n   Atom type %d %s  size %d  (%02x %02x %02x %02x)", atom->type,
                   constants_unmap(constants, atom->type), atom->size,p[0],p[1],p[2],p[3]);
...
void util_print_atom_sequence(int port,
                         const LV2_Atom_Sequence *aseq) {
   if (aseq->atom.type != constants.atom_Sequence) {
      printf("\nPort %d  UNEXPECTED ?    Atom sequence of type %d %s)",port,
             aseq->atom.type, constants_unmap(constants, aseq->atom.type));
      fflush(stdout);
   }
   ...
   if (aseq->atom.size > sizeof(LV2_Atom_Sequence)) {
      printf("\nPort %d  Atom sequence (type %d %s size  %d)", port,
