    ...
    if (subject == node->node_id) {
      ...
      if (!strcmp(key,"use_preset")) {
         ...
         ports_write(NULL, port_index, sizeof(float), 0U, &float_value);
          ...
          if (param_exists) {
              pw_log_info("Parameter:  key=%s  value=%s", key, value);
...
pw_log_info("node val %p",val);
              ...
              if (port_index == -1) {
                ...
                pw_log_info("Patch parameter using port %d", port_index);
