...
static void create_node_ports(Engine *engine) {
   ...
   for (int n = 0; n < arrlen(engine->host->ports); n++) {
      ...
      switch (host_port->type) {
         case HOST_CONTROL_INPUT:
            /*
            node_port->pwPort = pw_filter_add_port(engine->node->filter, PW_DIRECTION_INPUT,
         ...
         case HOST_CONTROL_OUTPUT:
            /*
            node_port->pwPort = pw_filter_add_port( engine->node->filter, PW_DIRECTION_OUTPUT,
            PW_FILTER_PORT_FLAG_MAP_BUFFERS, 0, pw_properties_new(PW_KEY_FORMAT_DSP, "control:f32",
            PW_KEY_PORT_NAME, host_port->name, NULL), NULL, 0); node_port->type =
            NODE_CONTROL_OUTPUT;
            */
         ...
         case HOST_ATOM_INPUT:
            node_port->pwPort = pw_filter_add_port(
                engine->node->filter, PW_DIRECTION_INPUT, PW_FILTER_PORT_FLAG_MAP_BUFFERS, 0,
         ...
         case HOST_ATOM_OUTPUT:
            node_port->pwPort = pw_filter_add_port(
                engine->node->filter, PW_DIRECTION_OUTPUT, PW_FILTER_PORT_FLAG_MAP_BUFFERS, 0,
                pw_properties_new(PW_KEY_FORMAT_DSP, "32 bit raw UMP", PW_KEY_PORT_NAME,
                                  host_port->name, NULL),
                NULL, 0);
...
int node_setup(Engine *engine) {
   ...
   sprintf(latency, "%d/%d", engine->node->latency_period, engine->samplerate);
   ...
   engine->node->filter = pw_filter_new_simple(pw_thread_loop_get_loop(engine->node->engine_loop),
                                              engine->enginename, props, &filter_events, engine);
   ...
   if (pw_filter_connect(engine->node->filter, PW_FILTER_FLAG_RT_PROCESS, params, 1) < 0) {
